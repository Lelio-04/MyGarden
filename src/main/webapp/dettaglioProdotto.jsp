...
<script>
  ...
  function aggiornaSidebar(carrello) {
	    ...
	    document.getElementById('sidebar-carrello').classList.add('attiva');
  ...
  function chiudiSidebar() {
	  document.getElementById('sidebar-carrello').classList.remove('attiva');
	}
...
<script>
  function aggiungiAlCarrelloDettaglio(productCode, maxQty) {
    ...
    fetch('GetCartServlet')
      ...
      .then(carrello => {
        ...
        if (currentQuantity + addingQuantity > maxQty) {
          alert(`Hai già ${currentQuantity} unità nel carrello. Disponibilità massima: ${maxQty}.`);
          throw new Error("Superata disponibilità");
        }
      ...
      .then(carrello => {
        aggiornaSidebar(carrello);
        document.getElementById('sidebar-carrello').style.display = 'block'; // Mostra sidebar
      })
